...
function compress() {
    return src([ '**/*', '!node_modules/**', '!./*.json', '!*.gitignore', '!gulpfile.js', '!todo.txt', '!.git/**', '!.vscode/**', '!assets/src/**' ]) 
    .pipe(zip('template-share-for-elementor.zip'))
    .pipe(dest('./'));
}
...
function browser(cb) {
    browserSync.init({
        proxy: "http://localhost/dev/wp-admin/"
    });
    cb();
}
